...
countries <- c("US", "Italy", "Iran", "India")
...
d_trend <- data.frame(diff(data.matrix(d_sums[,-1])))/(d_sums[-length(d_sums),-1])
...
d_trend$Value <- replace_na(d_trend$Value, 0)
#d_trend$Value <- replace(d_trend$Value,Inf,1)
